...
This script reads supabase_setup.sql and executes it.
It will prompt for connection details if not in environment.
"""
...
def load_sql_file():
    """Load SQL from supabase_setup.sql file."""
...
def get_connection_details():
    ...
    if not db_password:
        ...
        print("=" * 60)
        print()
        print("To set up the database tables, we need your database password.")
        print()
        ...
        print("  set SUPABASE_DB_PASSWORD=your_password && python setup_llm_logs_direct.py")
        print()
...
def execute_sql_via_psycopg2(conn_details, sql):
    ...
    try:
        ...
        # But psycopg2 can handle multi-statement SQL, so we'll execute it all at once
        try:
            cursor.execute(sql)
        ...
        tables_to_check = ["guidelines", "interactions", "llm_logs"]
        cursor.execute("""
            SELECT table_name 
        ...
        if missing_tables:
            ...
            print("[OK] All tables verified: guidelines, interactions, llm_logs")
        ...
        print("   - Check that your IP is allowed in Supabase Dashboard > Settings > Database > Connection Pooling")
        return False
...
def main():
    ...
    print("[i] Loaded SQL from supabase_setup.sql")
    ...
    if not conn_details:
        ...
        print("5. Click Run")
        print()
        print(f"[i] SQL file location: {Path('supabase_setup.sql').absolute()}")
        return 1
    
    # Execute SQL
    if execute_sql_via_psycopg2(conn_details, sql):
        print()
        print("=" * 60)
        print("Setup Complete!")
        print("=" * 60)
        print()
        print("All database tables are now set up:")
        print("  - guidelines (with version history)")
        print("  - interactions (conversation history)")
        print("  - llm_logs (LLM request/response logging)")
        print()
        print("LLM calls will now be automatically logged for debugging.")
        return 0
    else:
        print()
        print("=" * 60)
